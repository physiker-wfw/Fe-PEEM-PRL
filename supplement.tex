...
% \newcommand{\todo}[2][1]{\textsf{\slshape\color{red} > #2 < }}
...
For the calculations of the electronic structure and the photoemission intensities we used our computer program package \textsc{omni}, which is based on the spin-polarized relativistic layer Korringa-Kohn-Rostoker method (KKR; a brief introduction can be found in Ref.~\onlinecite{Henk02c}\nocite{Nalwa01a}). Solving the Dirac equation for a magnetic material, exchange splitting and spin-orbit coupling are treated on equal footing. \textsc{omni} has proven its quality in a number of publications, see for example Ref.~\onlinecite{tusche2018} and references therein.

The sample is taken as semi-infinite Fe(100), for which the muffin-tin potentials of the first seven layers deviate from that of bulk layers. For the image-potential barrier we take a smooth shape described in Ref.~\onlinecite{henk1993}.
