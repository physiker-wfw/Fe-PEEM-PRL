...
The reduced pulse length of x-ray free electron lasers (XFEL) can solve this issue \cite{Kutnyakhov20}. However, the low pulse repetition rate of the XFEL might lead to severe space-charge effects and additional measures need to be taken for a PEEM experiments \cite{Schoenhense18,Schoenhense21}. In strong contrast to the low repetition rates of an XFEL, we have combined PEEM with the MHz repetition rates of a high-power laser system as well as the Hg discharge lamp, thereby minimizing the space-charge effects and allow experiments in the laboratory. UV laser sources excite electrons from close to the Fermi level to energies slightly above the escape threshold. The reported MCD contrasts, especially for in-plane magnetization, are so small in threshold photoemission \cite{marx2000} that UV-PEEM has been discarded for magnetic domain imaging in the last two decades. Obviously, magnetic contrast needs to be increased for domain imaging with ultrashort laser pulses.
